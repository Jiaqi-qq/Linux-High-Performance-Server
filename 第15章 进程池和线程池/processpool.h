...
/* 描述一个子进程的类，m_pid屎目标子进程的PID，m_pipefd屎父进程和子进程通信用的管道 */
class process {
   public:
    process() : m_pid(-1) {}
...
/* 线程池构造函数。参数listenfd是监听socket，它必须在创建线程池之前被创建，否则子进程无法直接引用它。参数process_number指定进程池中子进程的数量 */
template <typename T>
...
/* 父进程中m_idx为-1，子进程中m_idx>=0，我们据此判断接下来运行的是父进程代码还是子进程代码 */
template <typename T>
...
void processpool<T>::run_child() {
    ...
    users = NULL;
    close(pipefd);
    // close(m_listenfd); /* 应该由m_listenfd的创建者来关闭这个文件描述符，即“对象（比如一个文件描述符或者一段堆内存）由哪个函数创建，就应该由哪个函数销毁” */
    close(m_epollfd);
}
                ...
                if (ret <= 0) {
                    ...
                    for (int i = 0; i < ret; ++i) {
                        switch (signals[i]) {
                            case SIGCHLD: {
                                ...
                                while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
                                    for (int i = 0; i < m_process_number; ++i) {
                                        /* 如果进程池中第i个子进程退出了，则主进程关闭相应的通信管道，并设置相应的m_pid为-1，以标记该子进程一经退出 */
                            ...
                            case SIGTERM:
                            case SIGINT: {
                                /* 如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知子进程结束更好的方法是向父、子进程之间的通信管道发送特殊数据，读者不妨自己实现 */
