    ...
    while (!stop_child) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if ((sockfd == connfd) && (events[i].events & EPOLLIN)) {
                ...
                /* 将客户数据读取到对应的读缓存中。该读缓存时共享内存的一段，它开始于idx*BUFFER_SIZE处，长度为BUFFER_SIZE字节。因此每个客户连接的读缓存是共享的
                 */
        ...
        for (int i = 0; i < number; ++i) {
            ...
            /* 某个子进程向父进程写入了数据 */
                ...
                if (ret == -1) {
                    ...
                    /* 向除负责第child个客户连接的子进程之外的其他子进程发送消息，通知他们有客户数据要写
                     */
