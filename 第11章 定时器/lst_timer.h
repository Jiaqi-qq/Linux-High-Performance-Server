...
public:
    ...
    void add_timer(util_timer* timer) {
        ...
        /* 如果目标定时器的超时时间小于当前链表中所有定时器的超时时间，则把该定时器插入链表头部，作为链表新的头节点。否则就需要调用重载函数add_timer(util_timer* timer, utol_timer* lst_head)，把它插入链表中合适的位置，以保证链表的升序特性 */
    ...
    void del_timer(util_timer* timer) {
        ...
        /* 如果链表中至少两个定时器，且目标定时器是链表的头节点，则将链表的头节点重置为原头结节点的下一个节点，然后删除目标定时器 */
        if(timer == head) {
            head = head->next;
            head->prev = nullptr;
            delete timer;
        ...
        /* 如果链表中至少有两个定时器，且目标定时器是链表的尾结点，则链表的尾结点重置为原尾结点的上一个节点，然后删除目标定时器 */
        if(timer == tail) {
            tail = tail->prev;
            tail->next = nullptr;
            delete timer;
    ...
    /* SIGALRM信号每次被触发就在其信号处理函数（如果使用统一事件源，则是主函数）中执行一次tick函数，以处理链表上的到期的任务 */
    void tick() {
        if(!head) {
            return;
        }
...
private:
    ...
    void add_timer(util_timer* timer, util_timer* lst_head) {
        ...
        /* 遍历lst_head节点之后的部分链表，直到找到一个超过时间大于目标定时器的超时时间的节点，并将目标定时器插入该节点之前 */
        while(tmp) {
